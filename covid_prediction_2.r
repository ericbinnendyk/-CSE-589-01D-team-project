...
# difference from previous: predicts whether number of cases has increased by at least 1.5 times
...
prepare.data <- function(case.data, num.weeks.ahead) {
  ...
  while (i <= 37) {
    ...
    if (week.data[i, 1] < week.data[i - num.weeks.ahead, 1]) {
      target_as_class = 'a';
    }
    if (week.data[i - num.weeks.ahead, 1] <= week.data[i, 1] && week.data[i, 1] < week.data[i - num.weeks.ahead, 1]*1.1) {
      target_as_class = 'b';
    }
    if (week.data[i - num.weeks.ahead, 1]*1.1 <= week.data[i, 1] && week.data[i, 1] < week.data[i - num.weeks.ahead, 1]*1.5) {
      target_as_class = 'c';
    }
    if (week.data[i - num.weeks.ahead, 1]*1.5 <= week.data[i, 1]) {
      target_as_class = 'd';
    }
    # countries with the most cases (in previous week) can be our features
    features = week.data[i - num.weeks.ahead, ];
    y[i - num.weeks.ahead] = target_as_class;
